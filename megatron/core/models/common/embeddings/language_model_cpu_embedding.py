...
class LanguageModelCPUEmbedding(MegatronModule):
    """Language model embeddings with weights on CPU.

    This class keeps embedding weights on CPU and performs embedding lookups on CPU,
    then moves the results to GPU. This is useful for reducing GPU memory usage when
    working with large vocabularies.
    ...
    def __init__(
        ...
        # Word embeddings (parallel) - kept on CPU
        # Force cpu_embedding to True in config for the embedding layer
        cpu_embedding_config = TransformerConfig(**{
            **config.__dict__,
            'cpu_embedding': True,
        ...
        self.word_embeddings = tensor_parallel.VocabParallelEmbedding(
            ...
            config=cpu_embedding_config,
            tp_group=self.tp_group,
        )

        # Position embedding (serial) - kept on CPU
        if self.add_position_embedding:
            # Create position embeddings on CPU
            self.position_embeddings = torch.nn.Embedding(
                self.max_sequence_length, self.config.hidden_size
            )
            # Move weights to CPU explicitly
            self.position_embeddings.weight.data = self.position_embeddings.weight.data.cpu()
        ...
        if self.num_tokentypes > 0:
            ...
            # Move weights to CPU explicitly
            self.tokentype_embeddings.weight.data = self.tokentype_embeddings.weight.data.cpu()
